...
- **`color.ts`** — Puyo colors as numeric constants using the `as const` + type extraction pattern (not TypeScript enums)
...
- **`hooks/useGraph.ts`** — Graph state management via `useReducer`. The `placeAndAddNode` action places a pair on the current board and adds the resulting state as a new node
- **`components/`** — UI components: `BoardView` (grid display with drop preview), `PairController` (movement/rotation controls), `PairSelector` (color picker), `GraphTreeView` (tree navigation with SVG thumbnails)
...
See `docs/puyo-rules.md` for detailed game rules. Key points for development:
